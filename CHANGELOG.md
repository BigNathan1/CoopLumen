...
- `GET /api/v1/accounts/:publicKey/trustlines` endpoint listing all non-native trustlines established by a Stellar account (#154).
- New `accountRouter` in `backend/src/api/routes/accounts.ts` mounted at `/accounts`; Zod validation on `publicKey`, Horizon errors mapped via `mapHorizonError` (#154).
