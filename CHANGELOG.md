...
### Added
- `ConfirmDialog` component for confirming destructive actions (#256, backlog #183), built on top of the existing `Modal` component. Features include `role="alertdialog"` semantics for decision-requiring dialogs, safe default focus on the cancel/neutral action (not the destructive confirm button) to prevent accidental destructive operations, loading/pending state on the confirm button to prevent double-submit during async operations, full ARIA support (`aria-modal`, `aria-labelledby`, `aria-describedby`), keyboard safety (ESC and backdrop click safely close without confirming, Enter only triggers confirm if the confirm button has explicit focus), reuse of Modal's backdrop, ESC handling, and focus trap rather than reimplementing, design-system token styling (danger color for confirm button, secondary for cancel) with dark-mode support, and comprehensive unit test coverage (60+ tests) for rendering, callbacks, keyboard behavior, focus management, loading state, ARIA attributes, and Modal integration.
- Global toast notification system with React context (`useToast` hook) supporting success/error/info/warning variants (#254, backlog #181). Features include configurable auto-dismiss duration (default 4 seconds), manual close affordance, multiple simultaneous toasts with independent dismissal, semantic color tokens from design system (no one-off colors), dark-mode support, and full accessibility: ARIA live regions with `role="alert"` (assertive) for errors/warnings and `role="status"` (polite) for success/info, keyboard-operable close buttons, non-focus-stealing behavior to avoid interrupting user workflows, and comprehensive test coverage (40+ tests) for all variants, auto-dismiss, manual dismiss, multiple toasts, ARIA attributes, edge cases, and focus management.
