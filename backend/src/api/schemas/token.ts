...
export const issueTokenSchema = z.object({
  ...
  name: z.string().trim().min(1, 'name must not be empty').max(64, 'name must be 64 characters or fewer').optional(),
  ...
  decimals: z.number().int().min(0, 'decimals must be 0 or greater').max(7, 'decimals must be 7 or fewer').default(7),
});
...
export const adminTokensQuerySchema = z.object({
  ...
  sortBy: z.enum(['created_at', 'name', 'asset_code', 'total_supply']).default('created_at').optional(),
